...
def FinalStateMomenta(m2_12, m2_23, m0, m1, m2, m3):
    ...
    sinθ2 = np.sqrt(1.0 - cosθ2*cosθ2)
    sinθ3 = np.sqrt(1.0 - cosθ3*cosθ3)

    #Fix 4-momenta with p3 oriented in z (quantisation axis) direction
    P1 = np.array([np.sqrt(p1_squared + m1 * m1), 0.0, 0.0, p1])
    P2 = np.array([np.sqrt(p2*p2 + m2*m2), p2*sinθ2, 0.0, -p2*cosθ2])
    P3 = np.array([np.sqrt(p3*p3 + m3*m3), -p3*sinθ3, 0.0, -p3*cosθ3])
...
def FinalStateAngles(P1, P2, P3): #P=[E, px, py, pz]
    ...
    PR1 = Rotation(0.0, -θR, 0.0, Lorentz(beta(PR), P1)) #PR1 = L(PRz)R(0,-θR,-φR)P1 = R(0,-θR,-φR,)L(PR)P1
    θR1 = np.arctan2(PR1[1], PR1[3])

    bP1 = beta(P1)
    g1=1/np.sqrt(1.0 - bP1[0]**2.0 - bP1[1]**2.0 - bP1[2]**2.0)

    bPR = beta(PR)
    bPR1 = beta(PR1)
    gR=1/np.sqrt(1.0 - bPR[0]**2.0 - bPR[1]**2.0 - bPR[2]**2.0)
    gR1=1/np.sqrt(1.0 - bPR1[0]**2.0 - bPR1[1]**2.0 - bPR1[2]**2.0)
    aR=np.arccos((1.0 + g1 + gR + gR1)**2/((1.0 + g1)*(1.0 + gR)*(1.0 + gR1))-1.0)

    #S resonance
    PS = P1 + P3
    θS = np.arctan2(PS[1], PS[3])

    PS1 = Rotation(0.0, -θS, 0.0, Lorentz(beta(PS), P1)) #PS1 = L(PSz)R(0,-θS,-φS)P1 = R(0,-θS,-φS,)L(PS)P1
    θS1 = np.arctan2(PS1[1], PS1[3])

    bPS = beta(PS)
    bPS1 = beta(PS1)
    gS=1/np.sqrt(1.0 - bPS[0]**2.0 - bPS[1]**2.0 - bPS[2]**2.0)
    gS1=1/np.sqrt(1.0 - bPS1[0]**2.0 - bPS1[1]**2.0 - bPS1[2]**2.0)
    aS=np.arccos((1.0 + g1 + gS + gS1)**2/((1.0 + g1)*(1.0 + gS)*(1.0 + gS1))-1.0)
